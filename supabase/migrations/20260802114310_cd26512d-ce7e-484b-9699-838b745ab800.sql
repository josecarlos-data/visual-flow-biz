DROP POLICY IF EXISTS "Objetivos visibles segun rol" ON public.objetivos;
CREATE POLICY "Objetivos visibles segun rol" ON public.objetivos
FOR SELECT TO authenticated
USING (
  public.is_approved(auth.uid()) AND (
    public.is_admin(auth.uid())
    OR public.has_role(auth.uid(), 'director_comercial')
    OR objetivos.vendedor = public.get_user_employee_code(auth.uid())
  )
);
...
REVOKE EXECUTE ON FUNCTION public.objetivos_seguimiento(integer) FROM PUBLIC, anon;
