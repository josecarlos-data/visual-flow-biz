...
BEGIN
  ...
  LOOP
    ...
    IF r.origen IS NOT NULL AND btrim(r.origen) <> '' THEN
      ...
      -- solo se considera marcador si la línea viene en mayúsculas
      IF raw1 = upper(raw1) AND coalesce(array_length(pal, 1), 0) > 0 THEN
        ...
        IF veredicto = 'pendiente' AND extensions.levenshtein(pal[1], 'NOCORRECTO') <= 3 THEN
          veredicto := 'NO CORRECTO'; consumidas := 1;
        ELSIF veredicto = 'pendiente' AND extensions.levenshtein(pal[1], 'CORRECTO') <= 2 THEN
      ...
      IF consumidas > 0 THEN
        ...
        -- párrafos íntegros en MAYÚSCULAS pegados al marcador -> nota de revisión
